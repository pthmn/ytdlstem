...
export default function KaraokePage() {
    ...
    const handleStart = async () => {
        ...
        try {
            const data = await startKaraoke(formData);
            setJob({
                jobId: data.job_id,
                status: data.status,
                progress: 0,
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const data = await getKaraokeStatus(job.jobId);
                setJob({
                    jobId: data.job_id,
                    status: data.status,
    ...
    return (
        <div className="page-container">
            <div className="page-header">
                <h1>Karaoke Maker</h1>
                <p>
                    Remove vocals from any song to create instrumental karaoke tracks.
                    Powered by AI.
                </p>
            </div>
            ...
            <div className="tabs">
                ...
                <button className={`tab ${inputMode === "search" ? "active" : ""}`} onClick={() => setInputMode("search")}>
                        ...
                        >
                            ...
                            <div className="upload-zone-text">
                                {file ? (
                                    ...
                                    <>Drag & drop an audio file here, or <strong>click to browse</strong></>
                                )}
                            </div>
                        </div>
                        <input
                    ...
                    {job.status === "done" && job.result && (
                        <>
                            ...
                            <div className="stem-grid">
                                {Object.entries(job.result.tracks).map(([track, filename]) => (
                                    <div key={track} className="stem-card glass-card">
                                        <div className="stem-card-title">
                                            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                                                <div className={`stem-icon ${track === "instrumental" ? "stem-instrumental" : "stem-vocals"}`}>
                                                    {track === "instrumental" ? "🎶" : "🎤"}
                                                </div>
                                                <h3>{track}</h3>
                                            </div>
                                            <a
                                                href={getKaraokeDownloadUrl(job.jobId, track)}
                                        ...
                                        <audio controls style={{ width: "100%" }}>
                                            <source src={getKaraokeDownloadUrl(job.jobId, track)} />
                                        </audio>
                                    </div>
                                ))}
                            </div>
